...
#### 🚀 Классическая установка (все в одном)
```bash
...
#### 🔧 Модульная установка (выборочно)
```bash
# 1. Клонировать репозиторий
git clone https://github.com/ncux-ad/Flame_Of_Styx_bot.git
cd Flame_Of_Styx_bot

# 2. Интерактивная модульная установка
bash scripts/install-modular.sh

# 3. Или установить конкретные модули
bash scripts/install-modular.sh core security monitoring

# 4. Или установить все модули сразу
bash scripts/install-modular.sh all
```
