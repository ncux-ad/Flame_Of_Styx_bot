...
print_header() {
    echo -e "${BLUE}🔨 Build Uptime Kuma Complete${NC}"
    echo -e "${BLUE}=============================${NC}"
}
...
# Проверяем сборку
print_step "Проверяем сборку..."
if [[ -f "dist/index.html" ]]; then
    print_success "Фронтенд собран успешно"
else
    print_error "Фронтенд не собран"
    print_info "Содержимое директории:"
    ls -la /opt/uptime-kuma/ | head -20
    exit 1
fi
...
sudo -u uptime-kuma timeout 15s node server/server.js &
TEST_PID=$!
...
print_success "🎉 Uptime Kuma полностью собран и работает!"
