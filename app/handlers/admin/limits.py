...
async def handle_setlimits_command(
    ...
    try:
        ...
        text += (
            f"📷 <b>Проверка медиа:</b> {'Включена' if limits.get('check_media_without_caption', False) else 'Отключена'}\n"
        )
        text += (
            f"🖼️ <b>Фото без подписи:</b> {'Разрешены' if limits.get('allow_photos_without_caption', False) else 'Запрещены'}\n"
        )
