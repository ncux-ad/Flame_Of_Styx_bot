...
async def handle_rate_limit_command(
    ...
    try:
        ...
        for config_name in ["user_messages", "admin_commands", "spam_analysis", "channel_management"]:
            try:
                ...
                if "error" not in info:
                    rate_limit_info.append({"config": config_name, "info": info})
        ...
        keyboard.add(InlineKeyboardButton(text="🔄 Обновить", callback_data="rate_limit_refresh"))
        keyboard.add(InlineKeyboardButton(text="🗑️ Сбросить все", callback_data="rate_limit_reset_all"))
        keyboard.add(InlineKeyboardButton(text="📊 Статистика", callback_data="rate_limit_stats"))
        keyboard.adjust(2)

        await send_silent_response(message, response_text, reply_markup=keyboard.as_markup())
...
async def handle_reset_rate_limit_command(
    ...
    try:
        ...
        if reset_count > 0:
            await send_silent_response(message, f"✅ Rate limit сброшен для {reset_count} конфигураций")
        else:
            await send_silent_response(message, "❌ Не удалось сбросить rate limit")
