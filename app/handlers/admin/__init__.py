...
# from .alerts import alerts_router  # Перемещено в основной роутер
...
# admin_router.include_router(alerts_router)  # Перемещено в основной роутер
...
# ============================================================================
# АЛЕРТЫ - перенесено из подроутера из-за проблем Aiogram 3.x
# ============================================================================


@admin_router.message(Command("alerts"))
...
@admin_router.message(Command("test_alert"))
async def handle_test_alert_command(message: Message, alert_service: AlertService) -> None:
    ...
    try:
        ...
        if success:
            await message.answer("✅ Тестовый алерт отправлен успешно!")
        else:
...
@admin_router.message(Command("alert_warning"))
async def handle_test_warning_alert_command(message: Message, alert_service: AlertService) -> None:
    ...
    try:
        success = await alert_service.send_warning_alert(
            ...
            data={"warning_type": "test", "confidence": 0.8},
        )

        if success:
            await message.answer("✅ Тестовый алерт-предупреждение отправлен!")
        else:
            await message.answer("❌ Ошибка отправки алерта-предупреждения")

    except Exception as e:
        await message.answer(f"❌ Ошибка тестирования алерта-предупреждения: {e}")


@admin_router.message(Command("alert_success"))
async def handle_test_success_alert_command(message: Message, alert_service: AlertService) -> None:
    ...
    try:
        success = await alert_service.send_success_alert(
            ...
            data={"operation": "test", "duration": "1s"},
        )

        if success:
            await message.answer("✅ Тестовый алерт об успехе отправлен!")
        else:
            await message.answer("❌ Ошибка отправки алерта об успехе")
