...
async def remove_suspicious_user_by_id(message: Message, profile_service: ProfileService, admin_id: int, user_id: int) -> None:
    ...
    try:
        ...
        logger.info(
            f"Removed user {sanitize_for_logging(str(user_id))} from suspicious profiles by {sanitize_for_logging(str(admin_id))}"
        )
        ...
        if command == "suspicious_analyze":
            ...
            if input_text.startswith("@"):
                ...
                try:
                    ...
                    await send_silent_response(
                        message, "❌ Поиск по username пока не поддерживается. Используйте ID пользователя."
                    )
                    return
                except Exception as e:
