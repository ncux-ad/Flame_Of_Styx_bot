...
async def handle_channel_message(
    ...
    try:
        ...
        logger.info(f"Channel handler debug: chat_id={message.chat.id}, sender_chat={message.sender_chat.id if message.sender_chat else None}, channel_id={channel_id}")
    ...
    try:
        ...
        logger.info(
            safe_format_message(
                ...
                old_status=sanitize_for_logging(update.old_chat_member.status if hasattr(update, 'old_chat_member') and update.old_chat_member else 'unknown'),
                new_status=sanitize_for_logging(update.new_chat_member.status if hasattr(update, 'new_chat_member') and update.new_chat_member else 'unknown')
            )
        )

        # Check if bot was added to a channel
        if (hasattr(update, 'new_chat_member') and 
            hasattr(update, 'old_chat_member') and
            update.new_chat_member and 
            update.old_chat_member):
            
            # Bot was added (was not member, now is member)
            if (update.old_chat_member.status == 'left' and 
            ...
            elif (update.old_chat_member.status in ['member', 'administrator'] and 
...
async def _notify_admin_bot_added(update, admin_id: int) -> None:
    ...
    try:
        chat = update.chat
        bot_username = update.bot.username or "your_bot"
