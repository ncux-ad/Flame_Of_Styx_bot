...
class RedisRateLimiter:
    ...
    def _setup_default_configs(self):
        ...
        self.configs = {
            "user_messages": RateLimitConfig(max_requests=10, window_seconds=60, key_prefix="user_msg"),
            "admin_commands": RateLimitConfig(max_requests=30, window_seconds=60, key_prefix="admin_cmd"),
            "spam_analysis": RateLimitConfig(max_requests=5, window_seconds=300, key_prefix="spam_analysis"),  # 5 минут
            "channel_management": RateLimitConfig(max_requests=20, window_seconds=60, key_prefix="channel_mgmt"),
        }
    ...
    async def check_rate_limit(self, config_name: str, identifier: str) -> RateLimitResult:
        """
        ...
        try:
            ...
            if current_count < config.max_requests:
                ...
                return RateLimitResult(allowed=False, remaining=0, reset_time=reset_time, retry_after=retry_after)
    ...
    async def get_rate_limit_info(self, config_name: str, identifier: str) -> Dict:
...
async def check_user_rate_limit(message: Message) -> RateLimitResult:
    ...
    return await rate_limiter.check_rate_limit("user_messages", str(message.from_user.id))
...
async def check_admin_rate_limit(message: Message) -> RateLimitResult:
    ...
    rate_limiter = await get_redis_rate_limiter()
    return await rate_limiter.check_rate_limit("admin_commands", str(message.from_user.id))
...
async def check_spam_analysis_rate_limit(message: Message) -> RateLimitResult:
    ...
    rate_limiter = await get_redis_rate_limiter()
    return await rate_limiter.check_rate_limit("spam_analysis", str(message.from_user.id))
