...
class LimitsService:
    ...
    def get_current_limits(self) -> Dict[str, Any]:
        ...
        return self._cached_limits or {
            ...
            "check_media_without_caption": self.config.check_media_without_caption,
            "allow_videos_without_caption": self.config.allow_videos_without_caption,
            "allow_photos_without_caption": self.config.allow_photos_without_caption,
            "max_document_size_suspicious": self.config.max_document_size_suspicious,
        }
    ...
    def get_limits_display(self) -> str:
        ...
        return (
            ...
            f"• Проверка медиа без подписи: {'✅' if limits['check_media_without_caption'] else '❌'}\n"
            f"• Разрешать GIF без подписи: {'✅' if limits['allow_videos_without_caption'] else '❌'}\n"
            f"• Разрешать фото без подписи: {'✅' if limits['allow_photos_without_caption'] else '❌'}\n"
            f"• Макс. размер документа для подозрения: {limits['max_document_size_suspicious']} байт\n\n"
