...
class InputValidator:
    ...
    def __init__(self):
        self.compiled_patterns = [
            (re.compile(pattern, re.IGNORECASE | re.DOTALL), code) for pattern, code in self.SUSPICIOUS_PATTERNS
        ]
    ...
    def _validate_text_content(self, text: str, field_name: str) -> List[ValidationError]:
        ...
        if text is None:
            errors.append(
                ValidationError(
                    field=field_name, message="Текст не может быть None", severity=ValidationSeverity.HIGH, code="text_is_none"
                )
            )
            return errors
        ...
        if not text.strip():
            errors.append(
                ValidationError(
                    field=field_name, message="Текст не может быть пустым", severity=ValidationSeverity.LOW, code="empty_text"
                )
            )
    ...
    def validate_phone_number(self, phone: str) -> List[ValidationError]:
        ...
        if not phone:
            errors.append(
                ValidationError(
                    field="phone", message="Отсутствует номер телефона", severity=ValidationSeverity.HIGH, code="missing_phone"
                )
            )
    ...
    def validate_email(self, email: str) -> List[ValidationError]:
        ...
        if not email:
            errors.append(
                ValidationError(
                    field="email", message="Отсутствует email", severity=ValidationSeverity.HIGH, code="missing_email"
                )
            )
