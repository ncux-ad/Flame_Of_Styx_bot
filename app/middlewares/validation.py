...
class ValidationMiddleware(BaseMiddleware):
    ...
    async def _sanitize_event_data(
        self, 
        event: TelegramObject, 
        data: Dict[str, Any]
    ) -> None:
    ...
    async def _log_event(
        ...
        if isinstance(event, Message):
            ...
            logger.info(
                f"Message validated: user_id={user_id}, chat_id={chat_id}, "
            ...
            logger.info(
                f"Callback query validated: user_id={user_id}, "
                f"data='{data_text[:100] if data_text else None}'"
            )
...
class CommandValidationMiddleware(BaseMiddleware):
    ...
    async def _handle_command_validation_error(
        self, 
        message: Message, 
        errors: List[str]
    ) -> None:
